  ...
  <div class="say-view animate__animated animate__fadeInUp">
    <div class="wrap">
      <div class="wrap-top">
        ...
        <div class="bg" :style="{backgroundImage:music?`url('${music?.json_content?.oneDataArticle?.poster}')` : ''}">
          <div class="img">
            <p>《{{music?.json_content?.oneDataArticle?.title}}》</p>
          </div>
        </div>
        <div class="t-section">
          <h1>{{music?.title}}</h1>
          ...
          <h3 v-html="music?.json_content?.content"
            style="font-size:15rem;color:#333;font-weight:400;line-height:30rem; "></h3>
          <!-- margin-bottom: 60rem; -->
          <div class="text">
            <p>{{music?.json_content?.editor}}</p>
            <p>{{music?.json_content?.copyright}}</p>
          </div>
          <FoundMovieViewComponent id="component" :author="author" :rela="rela" :praiseand="praiseand" />
        </div>
      </div>
      <div class="wrap-fot">
        <form action>
...
import FoundMovieViewComponent from "@/components/foundc/FoundMovieViewComponent.vue";
export default {
  components: {
    FoundMovieViewComponent
  },
  data() {
    return {
      music: null,
  ...
  methods: {
    getPlayList: function (id) {
      ...
      Promise.all([
        this.axios.get(`https://apis.netstart.cn/one/movie/htmlcontent/${id}`),
        this.axios.get(
          `https://apis.netstart.cn/one/author/list?content_id=${id}&category=5`
        ),
        this.axios.get(
          `https://apis.netstart.cn/one/relatedforwebview/movie/${id}`
        ),
        this.axios.get(
          `https://apis.netstart.cn/one/comment/praiseandtime/movie/${id}/0`
        )
