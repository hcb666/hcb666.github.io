<template>
  <li class="home-card" @click="$router.push({ path: '/playlist', query: { id: item.id } });"
    ...
    @update-playlistsong-list="$emit('update-playlistsong-list',$event)" :playing="playing" :songId="songId">
    <span> {{ item?.playCount | moneyFormat }}</span>
    <img :src="item ? `${item?.picUrl}?imageView=1&type=webp&thumbnail=370x0` : ''" alt="" />
    <h5>{{ item?.name }}</h5>
  </li>
</template>
